...
export function Insights() {
    ...
    const [selectedId, setSelectedId] = useState(() => spotlight[0]?.id ?? "");
    ...
    return (_jsxs("div", { className: "flex flex-col gap-6", children: [_jsx(PageHeader, { index: "06 / AI Insights", title: "AI insights", description: "Plain-language breakdown of the most concerning assessment in the fleet \u2014 why the model is worried, which signals drive it, and what to do next.", right: selectable.length ? (_jsx("select", { value: selectedId, onChange: (e) => setSelectedId(e.target.value), "aria-label": "Select assessment", className: "h-9 max-w-72 cursor-pointer rounded-lg border border-hairline bg-overlay/70 px-3 font-mono text-[11px] tabular-nums text-ink outline-none transition-colors focus:border-signal/60", children: selectable.map((x) => (_jsxs("option", { value: x.id, className: "bg-surface text-ink", children: [x.inputs.machineId, " \u00B7 ", fmtPercent(x.failureProbability), " \u00B7 ", fmtTs(x.ts)] }, x.id))) })) : undefined }), !assessments.length ? (_jsx(EmptyState, { icon: BrainCircuit, eyebrow: "No insight source", title: "No assessment to explain yet", description: "Run a machine assessment and it becomes the explainability source \u2014 the model maps raw signals into feature attribution, evidence, derived checks, and a plain-language why report.", action: _jsx(Link, { to: "/analyze", children: _jsx(Button, { variant: "primary", icon: _jsx(ArrowRight, { className: "h-4 w-4" }), label: "Run an assessment" }) }) })) : (selected && _jsx(InsightsReport, { a: selected }))] }));
}
