import type { AssessInput, ContributingFeature, FailureModeCode, FailureModeOutcome } from "@/lib/types";
...
const PRODUCT_FACTOR = { L: 0, M: 0.06, H: 0.14 } as const;

export interface SimulatedData {
  failureProbability: number;
  healthStatus: string;
  riskLevel: string;
  mode: FailureModeCode;
...
function sig(center: number, n: number, steep = 1): number {
  return clampP(1 / (1 + Math.exp(-steep * (n - center))));
}

export function simulateAssessment(input: AssessInput): SimulatedData {
  ...
  const union = Math.min(
    1,
    1 - (1 - twf) * (1 - hdf) * (1 - pwf) * (1 - osf) * (1 - rnf),
  );
  const failureProbability = round(clampP(union + 0.03 * product), 4);

  const scores: Record<FailureModeCode, number> = {
    TWF: twf,
    HDF: hdf,
    PWF: pwf,
    OSF: osf,
  ...
  const top = (Object.keys(scores) as FailureModeCode[]).filter((k) => k !== "NONE").sort((a, b) => scores[b] - scores[a])[0];
  const mode: FailureModeCode = failureProbability < 0.3 && scores[top] < 0.32 ? "NONE" : top;

  const modes: FailureModeOutcome[] = (Object.keys(scores) as FailureModeCode[])
    .filter((k) => k !== "NONE")
  ...
  const drivers: { key: string; label: string; value: number; magnitude: number; direction: "increases" | "decreases" }[] = [];
  if (twf > 0.3) drivers.push({ key: "tool_wear", label: "Tool wear", value: input.toolWear, magnitude: round(twf, 2), direction: "increases" });
  if (hdf > 0.3) drivers.push({ key: "temp_difference", label: "Temperature difference", value: round(d.tempDifference, 1), magnitude: round(hdf, 2), direction: "decreases" });
  if (pwf > 0.3) drivers.push({ key: "mechanical_power", label: "Mechanical power", value: round(d.mechanicalPower, 0), magnitude: round(pwf, 2), direction: "increases" });
  if (osf > 0.3) drivers.push({ key: "overstrain", label: "Overstrain", value: round(d.overstrain, 0), magnitude: round(osf, 2), direction: "increases" });
  const contributing = drivers.length ? drivers.slice(0, 4) : [{ key: "tool_wear", label: "Tool wear", value: input.toolWear, magnitude: 0.02, direction: "increases" as const }];
  ...
  if (healthStatus !== "Normal" && healthStatus !== "Warning") evidence.push("Urgency ranked by failure probability against the decision threshold.");
