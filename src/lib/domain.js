...
export const FAILURE_MODE_LIST = Object.values(FAILURE_MODES);
export const PRODUCT_TYPES = ["L", "M", "H"];
export function failureMode(code) {
    const key = Object.keys(FAILURE_MODES).includes(code)
        ? code
        : "NONE";
    return { code: FAILURE_MODES[key].code, name: FAILURE_MODES[key].name };
}
/* ---- risk mapping -------------------------------------------------- */
export function healthFromProbability(p) {
    if (p >= 0.8)
        return "Critical";
    if (p >= 0.6)
        return "High Risk";
...
export function riskFromProbability(p) {
    if (p >= 0.8)
        return "Critical";
    if (p >= 0.6)
        return "High";
...
export function priorityFromProbability(p) {
    if (p >= 0.8)
        return "Critical";
    if (p >= 0.6)
        return "High";
...
export function probabilityFromStatus(status) {
    switch (status) {
        case "Critical":
            return 0.85;
        case "High Risk":
...
export function flattenModes(modes) {
    return modes
