...
export function Topbar() {
  ...
  return (
    <header className="sticky top-0 z-20 border-b border-hairline bg-canvas/80 backdrop-blur-md">
      <div className="mx-auto flex h-16 w-full max-w-[1320px] items-center justify-between gap-4 px-4 sm:px-6 lg:px-10">
        <div className="flex min-w-0 items-center gap-3">
          ...
          <div className="min-w-0">
            ...
            <p className="mt-0.5 hidden truncate text-xs text-ink-2 sm:block">
              {current.to.replace("/", "").toUpperCase()}
            </p>
          </div>
        </div>

        <div className="flex items-center gap-3">
          <span className={cn("hidden font-mono text-xs tabular-nums text-ink-3 md:inline")}>{clock} UTC</span>
          <div
            role="status"
          ...
          >
            ...
            <span className="font-mono text-[10px] font-medium uppercase tracking-[0.14em] text-ink-2">
              {meta.label}
            </span>
          </div>
        </div>
